...
class LlamaAttention_MUSTAFAR(nn.Module):
    ...
    def dh_prune_value(self, value_states: torch.Tensor, target_sparsity = None, group_size = None): 
        if target_sparsity is None:
        ...
        # Compute the number of tokens to keep per group
        #num_to_keep = max(1, int((1 - target_sparsity) * group_size))
        num_to_keep = max(1, int((target_sparsity) * group_size))
        if DEBUG: print("NUM TO KEEP for Value: ", num_to_keep)

        # Ensure tensor is contiguous before reshaping
        value_states_grouped = value_states_grouped.contiguous()

        # Flatten across batch, heads, and hidden dim, keeping only the token groups separate
        value_states_flat = value_states_grouped.permute(0, 1, 4, 2, 3).reshape(-1, group_size)  # Shape: [(B * H * D * num_groups), group_size]

        # Compute pruning threshold per group of tokens
        ...
        # Reshape back to original dimensions
        pruned_value_states = pruned_value_states.reshape(B, H, D, num_groups, group_size).permute(0, 1, 3, 4, 2).reshape(B, H, T, D)

        # Remove padding if any was added
        if pad_size > 0:
            pruned_value_states = pruned_value_states[:, :, :T - pad_size, :]

        if DEBUG: 
                print("Debug: -- sparsity of just pruned value: ", self.calculate_sparsity(pruned_value_states))

        return pruned_value_states
    ...
    def dh_prune_value_old(self, iteration_num, attention_score:torch.Tensor, value_states: torch.Tensor, score_accumulator:torch.Tensor, target_sparsity = None, group_size = None):
        if target_sparsity is None:
            target_sparsity = self.v_sparsity
        if group_size is None:
            group_size = self.group_size
        ...
        if iteration_num == 0:
            ...
            sort_res = torch.sort(score.squeeze(2), dim=-2, descending=True)  # [B, H, token_length, D]
            ...
            if DEBUG: print("Debug: GQA: relevant_score_broadcast shape: ", relevant_score_broadcast.shape)
            if DEBUG: print("Debug: GQA: value_states_score_compute shape: ", value_states_score_compute.shape)
            if DEBUG: print("Debug: GQA: score_accumulator shape: ", score_accumulator.shape)
...
class LlamaFlashAttention_MUSTAFAR(LlamaAttention_MUSTAFAR):
    def forward(
        ...
        **kwargs,
        ...
        if value_score_accumulator is None:
            #value_score_accumulator = torch.zeros([bsz, self.num_heads, 2 * self.group_size, self.head_dim], dtype=torch.float32, device='cuda') #[B, H, self.group_size, D]
            value_score_accumulator = torch.zeros([bsz, self.num_heads // self.num_key_value_groups, 2 * self.group_size, self.head_dim], dtype=torch.float16, device='cuda') #[B, G, self.group_size, D]
        ...
        if past_key_value is not None:
            ...
            if (value_states_full.shape[-2] / self.residual_length) == 0:
                if (value_states_full.shape[-2] % self.group_size) == 0:
                #assert self.residual_length % self.group_size == 0
                    value_states_pruned = self.dh_prune_value(value_states_full[:,:,-self.group_size:,:]).contiguous() #pass in the last residual_length number of vectors. 
                    value_states_full = torch.cat((value_states_full[:,:,:-self.group_size,:], value_states_pruned), dim=2)
                else:
                    pass
            #the current context is larger than the residual length. 
            else: 
                #value_states_full : compressed + group_size filling up + residual_length full
                prefix = value_states_full[:, :, :-(self.residual_length), :]
                suffix = value_states_full[:, :, -(self.residual_length):, :]
                if (prefix.shape[-2] % self.group_size) == 0:
                    prefix_before_pruned = prefix[:, :, :-self.group_size, :]
                    prefix_pruned = self.dh_prune_value(prefix[:, :, -self.group_size:, :]).contiguous()
                    value_states_full = torch.cat((prefix_before_pruned, prefix_pruned, suffix), dim=2)
                else:
                    value_states_full = torch.cat((prefix, suffix), dim=2)
            ...
            #the current context is larger than the residual length. 
                ...
                if prefix.shape[-2] % self.group_size != 0:
                    if prefix.shape[-2] < self.group_size:
                        prefix_prune = None
                        prefix_full = prefix
                    else:
                        prefix_prune = prefix[:, :, :-(prefix.shape[-2] % self.group_size), :].contiguous()
                        prefix_full = prefix[:, :, -(prefix.shape[-2] % self.group_size):, :].contiguous()
                else:
                    prefix_prune = prefix
                    prefix_full = None
            
                if prefix_prune is not None:
                    if DEBUG: print("Debug:---- shape of value states TO PRUNE: ", prefix_prune.shape)  #[bsz, nh, t, hd]
                    if prefix_full is not None:
                        prefix_full = torch.cat((self.dh_prune_value(prefix_prune).contiguous(), prefix_full), dim=2)
                        if DEBUG: print("Debug:---- Value Prune complete")
                    else:
                        prefix_full = self.dh_prune_value(prefix_prune)
                value_states_full = torch.cat((prefix_full, suffix), dim=2)
