...
from transformers.models.llama.modeling_llama import LlamaRotaryEmbedding, LlamaMLP, LlamaRMSNorm
...
class LlamaAttention_MUSTAFAR(nn.Module):
    ...
    def key_pruner_query_driven(self, kv_states, q_states, ratio=0.5):
        #print("Debug--: key_pruner_query_driven")
        #print(kv_states)
        ratio = self.k_sparsity
        _, QH, _, _, = q_states.shape
        bz, KH, seqlen, head_dim = kv_states.shape
        num_groups = QH // KH
        k = int(head_dim * ratio)
        # new implementation
        queries_norm = torch.pow(q_states[..., -32:, :], 2).mean(dim=2) #token-dimension mean.
        queries_grouped = queries_norm.view(bz, KH, num_groups, head_dim)
        queries_norm = queries_grouped.mean(dim=2)
        ...
        mask_k = mask.unsqueeze(2).expand(-1, -1, seqlen, -1)

        #return kv_states[~mask_k].reshape(bz,-1,seqlen,head_dim-k) #, ~mask
        # Zero out masked vectors instead of reshaping
        kv_states = kv_states.clone()  # Create a copy to avoid modifying original
        kv_states[mask_k] = 0  # Zero out the masked positions

        if DEBUG: print("Debug: -- sparsity of just pruned key: ", self.calculate_sparsity(kv_states.view(bz, KH, seqlen, head_dim)))

        
        return kv_states  # Shape remains [bz, _, seqlen, head_dim]
    
    def value_pruner_attention_driven(self, kv_states, attention_score, ratio=0.5):
        #print("Debug--: key_pruner_query_driven")
        #print(kv_states)
        ratio = self.v_sparsity
        _, QH, _, _, = attention_score.shape
        bz, KH, seqlen, head_dim = kv_states.shape
        num_groups = QH // KH
        k = int(head_dim * ratio)
        # new implementation    
        ...
        #attention_norm = torch.pow(attention_score, 2).mean(dim=2) #already recomputed and fed in as [:, :, :, -32:]. 
        #attention_norm = attention_norm.view(bz, KH, num_groups, head_dim)
        #attention_norm = attention_norm.mean(dim=2)
        
        keys_norm = torch.pow(kv_states, 2).mean(dim=2)
        #key = queries_norm * keys_norm
        key = keys_norm
        #key = attention_norm * keys_norm
        # key = torch.matmul(q_states[..., -32:, :].permute(0, 1, 3, 2).unsqueeze(-1), kv_states.transpose(2, 3).unsqueeze(-2)).pow_(2).sum(dim=(-1, -2))
        #del q_states
        _, indices = torch.topk(key, k, dim=-1, largest=False)  
        keep_idx = indices.sort().values
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            if (key_states.shape[-2] / self.residual_length) == 0:
                ...
                key_states_full = torch.cat([prefix_pruned, suffix], dim=2) 
            '''
            key_states_full = self.key_pruner_query_driven(key_states, query_states)
            ...
            #_, value_states_full = self.dh_prune_value(self.generation_count, attn_weights, value_states, value_score_accumulator)
            #value_states_full = value_states
            value_states_full = self.value_pruner_attention_driven(value_states, attn_weights)
